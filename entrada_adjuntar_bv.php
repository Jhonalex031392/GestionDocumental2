...
$entrada_adjuntar_blobview = NULL; // Initialize page object first

class centrada_adjuntar_blobview extends centrada {
	...
	// Page object name
	var $PageObjName = 'entrada_adjuntar_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('entrada', 'x_adjuntar');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('adjuntar');
				...
				if (trim(strval($this->Recordset->fields('adjuntar'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('adjuntar') . "\"");
				}
...
// Create page object
if (!isset($entrada_adjuntar_blobview)) $entrada_adjuntar_blobview = new centrada_adjuntar_blobview();

// Page init
$entrada_adjuntar_blobview->Page_Init();

// Page main
$entrada_adjuntar_blobview->Page_Main();
?>
<?php
$entrada_adjuntar_blobview->Page_Terminate();
?>
