...
class cusuarios extends cTable {
	...
	function __construct() {
		...
		// contra
		$this->contra = new cField('usuarios', 'usuarios', 'x_contra', 'contra', '`contra`', '`contra`', 200, -1, FALSE, '`contra`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['contra'] = &$this->contra;

		// tipo_usuario
		$this->tipo_usuario = new cField('usuarios', 'usuarios', 'x_tipo_usuario', 'tipo_usuario', '`tipo_usuario`', '`tipo_usuario`', 3, -1, FALSE, '`tipo_usuario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->tipo_usuario->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
	...
	function InsertSQL(&$rs) {
		...
		foreach ($rs as $name => $value) {
			if (!isset($this->fields[$name]))
				continue;
			if (EW_ENCRYPTED_PASSWORD && $name == 'contra')
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('usuario', $rs))
				ew_AddFilter($where, ew_QuotedName('usuario') . '=' . ew_QuotedValue($rs['usuario'], $this->usuario->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@usuario@", ew_AdjustSql($this->usuario->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->usuario->CurrentValue)) {
			$sUrl .= "usuario=" . urlencode($this->usuario->CurrentValue);
		} else {
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->contra->Exportable) $Doc->ExportCaption($this->contra);
				if ($this->tipo_usuario->Exportable) $Doc->ExportCaption($this->tipo_usuario);
			} else {
				if ($this->usuario->Exportable) $Doc->ExportCaption($this->usuario);
				if ($this->contra->Exportable) $Doc->ExportCaption($this->contra);
				if ($this->tipo_usuario->Exportable) $Doc->ExportCaption($this->tipo_usuario);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->contra->Exportable) $Doc->ExportField($this->contra);
					if ($this->tipo_usuario->Exportable) $Doc->ExportField($this->tipo_usuario);
				} else {
					if ($this->usuario->Exportable) $Doc->ExportField($this->usuario);
					if ($this->contra->Exportable) $Doc->ExportField($this->contra);
					if ($this->tipo_usuario->Exportable) $Doc->ExportField($this->tipo_usuario);
