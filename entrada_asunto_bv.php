...
$entrada_asunto_blobview = NULL; // Initialize page object first

class centrada_asunto_blobview extends centrada {
	...
	// Page object name
	var $PageObjName = 'entrada_asunto_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('entrada', 'x_asunto');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('asunto');
				...
				if (trim(strval($this->Recordset->fields('asunto'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('asunto') . "\"");
				}
...
// Create page object
if (!isset($entrada_asunto_blobview)) $entrada_asunto_blobview = new centrada_asunto_blobview();

// Page init
$entrada_asunto_blobview->Page_Init();

// Page main
$entrada_asunto_blobview->Page_Main();
?>
<?php
$entrada_asunto_blobview->Page_Terminate();
?>
