...
//
// Table class for seguimiento
//
class cseguimiento extends cTable {
	var $numero;
	var $radicado;
	var $fecha;
	var $hora;
	...
	function __construct() {
		...
		// numero
		$this->numero = new cField('seguimiento', 'seguimiento', 'x_numero', 'numero', '`numero`', '`numero`', 3, -1, FALSE, '`numero`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->numero->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['numero'] = &$this->numero;

		// radicado
		$this->radicado = new cField('seguimiento', 'seguimiento', 'x_radicado', 'radicado', '`radicado`', '`radicado`', 200, -1, FALSE, '`radicado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['radicado'] = &$this->radicado;

		// fecha
		$this->fecha = new cField('seguimiento', 'seguimiento', 'x_fecha', 'fecha', '`fecha`', 'DATE_FORMAT(`fecha`, \'%Y/%m/%d\')', 133, 5, FALSE, '`fecha`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fecha->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['fecha'] = &$this->fecha;

		// hora
		$this->hora = new cField('seguimiento', 'seguimiento', 'x_hora', 'hora', '`hora`', 'DATE_FORMAT(`hora`, \'%Y/%m/%d\')', 134, -1, FALSE, '`hora`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->hora->FldDefaultErrMsg = $Language->Phrase("IncorrectTime");
		$this->fields['hora'] = &$this->hora;

		// dependencia
		$this->dependencia = new cField('seguimiento', 'seguimiento', 'x_dependencia', 'dependencia', '`dependencia`', '`dependencia`', 200, -1, FALSE, '`dependencia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['dependencia'] = &$this->dependencia;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("seguimientoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("seguimientoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "seguimientoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("seguimientoedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("seguimientoadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("seguimientodelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->dependencia->Exportable) $Doc->ExportCaption($this->dependencia);
			} else {
				if ($this->numero->Exportable) $Doc->ExportCaption($this->numero);
				if ($this->radicado->Exportable) $Doc->ExportCaption($this->radicado);
				if ($this->fecha->Exportable) $Doc->ExportCaption($this->fecha);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->dependencia->Exportable) $Doc->ExportField($this->dependencia);
				} else {
					if ($this->numero->Exportable) $Doc->ExportField($this->numero);
					if ($this->radicado->Exportable) $Doc->ExportField($this->radicado);
					if ($this->fecha->Exportable) $Doc->ExportField($this->fecha);
