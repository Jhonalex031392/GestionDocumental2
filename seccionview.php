...
$seccion_view = NULL; // Initialize page object first

class cseccion_view extends cseccion {
	...
	function __construct() {
		...
		if (@$_GET["codigo_seccion"] <> "") {
			$this->RecKey["codigo_seccion"] = $_GET["codigo_seccion"];
			$KeyUrl .= "&codigo_seccion=" . urlencode($this->RecKey["codigo_seccion"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["codigo_seccion"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["codigo_seccion"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["codigo_seccion"] <> "") {
				...
				$this->RecKey["codigo_seccion"] = $this->codigo_seccion->QueryStringValue;
			} else {
				$sReturnUrl = "seccionlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "seccionlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "seccionlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_seccion\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_seccion',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fseccionview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($seccion_view)) $seccion_view = new cseccion_view();

// Page init
$seccion_view->Page_Init();

// Page main
$seccion_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$seccion_view->Page_Render();
?>
...
// Page object
var seccion_view = new ew_Page("seccion_view");
seccion_view.PageID = "view"; // Page ID
var EW_PAGE_ID = seccion_view.PageID; // For backward compatibility

// Form object
var fseccionview = new ew_Form("fseccionview");

// Form_CustomValidate event
fseccionview.Form_CustomValidate = 
...
fseccionview.ValidateRequired = true;
<?php } else { ?>
fseccionview.ValidateRequired = false; 
<?php } ?>
...
<?php $seccion_view->ExportOptions->Render("body") ?>
<?php if (!$seccion_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($seccion_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $seccion_view->ShowPageHeader(); ?>
<?php
$seccion_view->ShowMessage();
?>
<form name="fseccionview" id="fseccionview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="seccion">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_seccionview" class="table table-bordered table-striped">
<?php if ($seccion->codigo_seccion->Visible) { // codigo_seccion ?>
	<tr id="r_codigo_seccion"<?php echo $seccion->RowAttributes() ?>>
		<td><span id="elh_seccion_codigo_seccion"><?php echo $seccion->codigo_seccion->FldCaption() ?></span></td>
...
<?php echo $seccion->codigo_seccion->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
...
<?php echo $seccion->idsede->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
...
<?php echo $seccion->nombre_seccion->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
</table>
